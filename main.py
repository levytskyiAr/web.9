...
def load_data_to_database():
    ...
    with open('C:\\Projects\\web.9\\authors.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
        for el in data:
            try:
    ...
    # Завантаження цитат
    with open('C:\\Projects\\web.9\\quotes.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
        for el in data:
            authors = list(Author.objects(fullname=el.get('author')))
            if authors:
                author = authors[0]
            else:
                print(f"Автора з іменем {el.get('author')} не знайдено в базі даних.")
                continue
